...
local function drawStatus()
  ...
  message = message .. string.char( 30 ) .. string.char( 31 )
  term.write(message)
  end
end
...
local function viewItem()
  ...
  if e.virtual and e.vType=="peripheral_item" then
    local p_info = getPeripheralInfo(e.name)
    info = {}
    for _,line in ipairs(p_info) do table.insert(info,line) end
...
local function handleKey(k)
  ...
  if viewOverlay or treeOverlay or helpOverlay then
    if k==keys.backspace then viewOverlay=nil; treeOverlay=nil; helpOverlay=nil end
    return
  end
  ...
  elseif k==keys.backspace then goUp(p)
  ...
  elseif k == keysCfg.HELP then
    showHelp()
  elseif k==keysCfg.QUIT then term.clear() os.exit() end
end
